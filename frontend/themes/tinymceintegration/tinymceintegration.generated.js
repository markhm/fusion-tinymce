...
window.Vaadin['_vaadintheme_tinymceintegration_globalCss'] = window.Vaadin['_vaadintheme_tinymceintegration_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_tinymceintegration_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_tinymceintegration_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_tinymceintegration_globalCss'].push(target);
  }
  if (!document['_vaadintheme_tinymceintegration_componentCss']) {
    
    document['_vaadintheme_tinymceintegration_componentCss'] = true;
  }
